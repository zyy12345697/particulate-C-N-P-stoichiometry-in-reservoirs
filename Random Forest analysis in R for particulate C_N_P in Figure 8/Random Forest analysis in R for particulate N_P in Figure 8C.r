...
otu <- read.delim('Data-RF in R for particulate N_P.txt', row.names = 1)
...
set.seed(123)
otu_forest <- randomForest(PON_PP~., data = otu, importance = TRUE, ntree = 500)
otu_forest
...
p <- p +
  annotate('text', label = 'PON_PP', x = 2, y = 15, size = 4) +
  annotate('text', label = sprintf('italic(R^2) == %.2f', 0.43), x = 2, y = 13, size = 3, parse = TRUE)
...
set.seed(123)
otu_rfP <- rfPermute(PON_PP~., data = otu, importance = TRUE, ntree = 500, nrep = 1000, num.cores = 1)
otu_rfP
